...
def calcula_taxa_b3(serie_mtm: pd.DataFrame) -> pd.DataFrame:
    ...
    if not datas_b3.empty:
        ...
        custos_b3["custo"] = custos_b3["custo"]+((len(datas)*(0.1/100))/126)*custos_b3['MTM'].values[-1]

        #custos proporcionais no inicio do semestre
        datas = ettj.listar_dias_uteis(inicio_primeiro_semestre, fim_primeiro_semestre)
        custos_b3["custo"] = custos_b3["custo"]+((len(datas)*(0.1/100))/126)*custos_b3["MTM"].values[0]
    else:
        ...
        fim = "/".join([dia, mes, ano])
        datas = ettj.listar_dias_uteis(inicio, fim)
        custos_b3 = pd.DataFrame(np.array([serie_mtm.iloc[-1].values[0], serie_mtm.iloc[-1].values[0]*(len(datas)*0.1/100)/126]), 
                                index=["MTM", "custo"], columns=[hoje]).T
    return custos_b3
...
def _get_valid_date(date: datetime, carteira_tesouro_direto: pd.DataFrame()) -> datetime:
    if isinstance(date, str): date = pd.to_datetime(date)
    count=0
    df = carteira_tesouro_direto[carteira_tesouro_direto.index.isin([date+timedelta(days=count)])]
    while df.empty:
        count+=1
        df = carteira_tesouro_direto[carteira_tesouro_direto.index.isin([date.date()+timedelta(days=count)])]
    return date+timedelta(days=count)

def movimentacoes_titulos_publicos(tipo_movimentacao: str, 
    proxies: Optional[Dict[str, str]] = None, 
    excluir: List[str] = ["Juros Semestrais", "Educa+", "RendA+"],
    filtrar_data: Union[str, None] = None) -> pd.DataFrame:
    '''
    ...
    taxa_agrupada = busca_tesouro_direto(tipo=tipo_movimentacao, proxies=proxies, agrupar=True)
    movimentacao = taxa_agrupada.reset_index().groupby(["Tipo Titulo","Vencimento do Titulo", f"Data {tipo_movimentacao.title()}"], as_index=False)[["Quantidade"]].sum()
    if filtrar_data:
        dt_investimento = pd.to_datetime(filtrar_data)
        movimentacao = movimentacao[(movimentacao["Vencimento do Titulo"]>dt_investimento) &
                                (movimentacao[f"Data {tipo_movimentacao.title()}"]>dt_investimento)]
    movimentacao = movimentacao.set_index(["Tipo Titulo", "Vencimento do Titulo"]).reset_index()
    movimentacao["Vencimento do Titulo"] = movimentacao["Vencimento do Titulo"].astype(str)
    if excluir:
        df1 = pd.DataFrame()
    ...
    movimentacao["Titulo"] = movimentacao["Tipo Titulo"] + "_" + movimentacao["Vencimento do Titulo"]
    movimentacao = movimentacao.drop(["Tipo Titulo", "Vencimento do Titulo"], axis=1)
    movimentacao_pivot = pd.pivot_table(movimentacao.reset_index(), values="Quantidade", 
                                        index=f"Data {tipo_movimentacao.title()}", columns="Titulo")
    return movimentacao_pivot

def calcula_retorno_titulo(tipo_titulo: str, vencimento: str, 
    data_investimento: str, investimento: str, proxies: Optional[Dict[str, str]] = None) -> pd.DataFrame:
    '''
...
def calcula_retorno_carteira(carteira: Type(Carteira), 
    ...
    for tpf in carteira.titulos:
        ...
        serie_retorno = calcula_retorno_titulo(tipo_titulo, vencimento, data_investimento, investimento, proxies=proxies)
        carteira_tesouro_direto = pd.concat([carteira_tesouro_direto, serie_retorno], axis=1)
    ...
    investimentos = [pd.to_datetime(carteira.titulos[x]["Data Investimento"]) for x in range(len(carteira.titulos))]
    investimentos_datas_validas = [_get_valid_date(x, carteira_tesouro_direto) for x in investimentos]
    for x, idx in enumerate(carteira_tesouro_direto.index[1:], start=1):
        idx_investimento = [i for i, date in enumerate(investimentos_datas_validas) if idx==date]
        if idx_investimento:
            idx_investimento = idx_investimento[0]
            dt_investimento = [investimentos_datas_validas[idx_investimento]]
            ativo = [carteira.titulos[x]["Nomeclatura"]+"_"+carteira.titulos[x]["Vencimento"][:4]+"_"+str(investimentos[idx_investimento])[:-9] for x in range(len(carteira.titulos)) if pd.to_datetime(carteira.titulos[x]["Data Investimento"])==investimentos[idx_investimento]]
            investimento = carteira_tesouro_direto.loc[dt_investimento, ativo].values[0][0]
            carteira_tesouro_direto.loc[dt_investimento, "Qde Cotas"] = carteira_tesouro_direto.iloc[x-1]["Qde Cotas"] + investimento/carteira_tesouro_direto.iloc[x-1]["Cotas"]
        else:
            carteira_tesouro_direto.loc[idx, "Qde Cotas"] = carteira_tesouro_direto.iloc[x-1]["Qde Cotas"]
        carteira_tesouro_direto.loc[idx, "Cotas"] = carteira_tesouro_direto.loc[idx, columns].dropna().sum()/carteira_tesouro_direto.loc[idx, "Qde Cotas"]
    carteira_tesouro_direto["Rentabilidade Diária"] = carteira_tesouro_direto["Cotas"].pct_change()
    carteira_tesouro_direto["Rentabilidade Acumulada"] = ((1 + carteira_tesouro_direto["Rentabilidade Diária"]).cumprod() - 1)
