...
def nomeclatura_titulos() -> Dict[str,str]:
    nomeclatura_dict = {'Tesouro IPCA+ com Juros Semestrais': "NTN-B",
...
class Titulo:
    def __init__(self, tipo_titulo=None, data_vencimento=None, data_investimento=None, quantidade=None):
        ...
        self.titulo = {"Tipo": self.tipo_titulo,
...
def busca_tesouro_direto(tipo:str="venda", proxies:Optional[Dict[str,str]]=None, agrupar:bool=False):
    if tipo.lower()=="venda":
    ...
    coluna_datas = [x for x in df.columns if x.startswith("Data") or x.startswith("Vencimento")]
    if coluna_datas:
...
def calcula_taxa_b3(serie_mtm:pd.DataFrame)->pd.DataFrame:
    ...
    datas_b3 = datas_b3[(datas_b3.index.month==1) | (datas_b3.index.month==7)] #meses de pagamento à B3
    
    custos_b3 = datas_b3.copy()
    custos_b3["custo"] = [custos_b3.loc[x,:].values[0]*(0.25/100) if x<pd.to_datetime("2022-01-01") else custos_b3.loc[x,:].values[0]*(0.2/100)
                          for x in custos_b3.index]#era 0,25% e a partir de jan/22 será 0,2%aa
    return custos_b3

def get_custos(serie_mtm:pd.DataFrame, investimento:float, data_investimento:str, data_resgate:str, custo_b3:bool=False):
    ...
    detalhamento_custos = {"IRPF":rendimento*irpf,
...
def calcula_mtm_titulo(tipo_titulo:str, quantidade:float, data_investimento:str, vencimento:str, proxies:Optional[Dict[str,str]]=None)->Union[pd.DataFrame, pd.DataFrame]:
    taxa_agrupada = busca_tesouro_direto(tipo="taxa", proxies=proxies, agrupar=True)
    titulo = taxa_agrupada.loc[(tipo_titulo, vencimento)]
    serie_mtm = (titulo.sort_values("Data Base").set_index("Data Base")[["PU Base Manha"]]*quantidade)
...
def calcula_mtm_carteira(carteira_tesouro_direto:pd.DataFrame)->Union[pd.DataFrame, pd.DataFrame]:
...
def plot_taxas(tipo_titulo:str, data_investimento:str, vencimento:str, proxies:Optional[Dict[str,str]]=None)->None:
    taxa_agrupada = busca_tesouro_direto(tipo="taxa", proxies=proxies, agrupar=True)
    titulo = taxa_agrupada.loc[(tipo_titulo, vencimento)]
    taxa = titulo.sort_values("Data Base").set_index("Data Base")[["Taxa Compra Manha", "Taxa Venda Manha"]]
...
def plot_mtm(serie_mtm:pd.DataFrame, data_investimento:str, investimento:float)->None:
    plt.figure(figsize=(16,5))
    plt.plot(serie_mtm)
    plt.axvline(pd.to_datetime(data_investimento), color="red")
